...
## More commands for survival
- The `date` command displays or sets time and date.
...
- The `uname` ⇒ unix name command prints the operating system name.
- The `who` command displays the list of users currently logged on to the system.

## Compressing & Decompressing
- The ZIP format is not very common on Unix systems. Mostly, **tarballs (tar.gz)** are used.
- A tarball is made up of an archive (tar) and a compression (gzip).
- It is also possible to use other compression formats, like bzip2.
- To create a tarball, use the tar command with options `-c`, `-z`, `-f`.
    - `c`: create new archive
    - `z`: using gzip
    - `f`: file path  
    > <ins>Ex:</ins> `tar -czf tarball.tar.gz file1 file2` create tarball.tar.gz included with file1 and file2 files.
- Using the `-x` option the archive can be extracted.  
    > <ins>Ex:</ins> `tar -xzf tarball.tar.gz` extract tarball.tar.gz.
- Using the `-v` option the contents of a tarball can be listed.  
    > <ins>Ex:</ins> `tar -tvf tarball.tar.gz` list of contents fo tarball.tar.gz.
- Familiar ZIP compression can be used as well.  
    > <ins>Ex:</ins> `zip file.zip file1 file2` create file.zip included with file1 and file2 files.
- Can be uncompressed with unzip.  
    > <ins>Ex:</ins> `unzip file.zip` unzip/extract file.zip
- If compressing a directiory with all subdirectories and files is wanted, then the recursive option with zip must be used.  
    > <ins>Ex:</ins> `zip -r file.zip theDirectory` unzip/extract file.zip